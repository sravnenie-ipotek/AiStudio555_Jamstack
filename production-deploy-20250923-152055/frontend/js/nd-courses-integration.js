...
(function() {
    ...
    async function populateCoursesSection(coursesData) {
        ...
        for (const container of tabContainers) {
            ...
            const tabPane = container.closest('.featured-courses-tab-pane');
            const tabElement = tabPane ? tabPane.getAttribute('data-w-tab') : null;
            let coursesToShow = courses;

            // For initial load, show all courses in Tab 1, empty in others
            if (tabElement && tabElement !== 'Tab 1') {
                // Other tabs will be populated when clicked
                coursesToShow = [];
            }
