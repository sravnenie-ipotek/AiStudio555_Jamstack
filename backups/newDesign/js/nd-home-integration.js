...
(function() {
    ...
    function updateCategoryItems(categories) {
        ...
        // Use proper Webflow course categories structure
        container.className = 'course-categories-collection-list w-dyn-items';
        ...
        limitedCategories.forEach((category, index) => {
            ...
            // Create card with proper Webflow structure
            const categoryCard = document.createElement('a');
            categoryCard.className = 'course-categories-single w-inline-block';
            categoryCard.setAttribute('href', category.url || '#');
            categoryCard.setAttribute('data-category', category.name.toLowerCase().replace(/\s+/g, '-'));

            // Create short, concise descriptions like the original design
            const shortDescriptions = {
                'Web Development': 'Build Dynamic Websites',
                'App Development': 'Create Mobile Apps',
                'Mobile Development': 'Create Mobile Apps',
                'Machine Learning': 'Implement Deep Learning',
                'Cloud Computing': 'Manage Cloud Services'
            };

            const shortDesc = shortDescriptions[category.name] || category.name;
            ...
            categoryCard.innerHTML = `
                ...
                <div class="course-categories-typography">
                    <div class="course-categories-icon-wrapper">
                        <img src="${category.icon || 'images/placeholder-icon.svg'}"
                             alt="${category.name}"
                             class="course-categories-icon">
                    </div>
                    <h4 class="course-categories-name">${category.name}</h4>
                    <p class="course-categories-description-text">${shortDesc}</p>
                </div>
            `;

            // No text overflow detection needed for the simple layout

            // Simple structure - no expandable descriptions needed
