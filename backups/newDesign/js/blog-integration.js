...
const BlogIntegration = {
    ...
    loadBlogPosts: async function(page = 1) {
        try {
            ...
            console.log('Number of posts received:', result.data ? result.data.length : 0);
            ...
            if (posts.length === 0) {
                ...
                console.log('Rendering', posts.length, 'posts from database');
    ...
    normalizePost: function(item) {
        ...
        return {
            ...
            image: item.featured_image_url || item.image_url || item.featured_image || item.image || this.getRandomImage(),
            url: item.url || '#',
    ...
    createBlogCard: function(post, index) {
        ...
        const safePost = {
            ...
            // CHANGE: Use detail page URL instead of external URL (following course pattern)
            detailUrl: `detail_blog.html?id=${post.id}`,
            externalUrl: post.url || '#'
        };

        const cardHtml = `
            <div class="uniform-card">
                <a href="${safePost.detailUrl}" class="uniform-card-image-link">
                ...
                <div class="uniform-card-content">
                    ...
                    <div class="uniform-card-action">
                        <a href="${safePost.detailUrl}" class="uniform-card-button">
