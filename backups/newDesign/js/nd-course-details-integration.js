...
(function() {
    ...
    function populateCourseDetails(course) {
        ...
        // Course Objectives / "What You will Get"
        if (course.objectives || course.what_you_learn) {
            const objectivesContainer = document.querySelector('.get-video-featured-list-wrapper');
            if (objectivesContainer) {
                const objectives = course.objectives ?
                    course.objectives.split(',').map(obj => obj.trim()) :
                    (Array.isArray(course.what_you_learn) ? course.what_you_learn : []);

                if (objectives.length > 0) {
                    objectivesContainer.innerHTML = objectives.map(obj =>
                        `<div class="get-video-featured-list-single">
                            <img src="images/Get-Video-Featured-List-Icon.svg" loading="lazy" alt="" class="get-video-featured-list-icon">
                            <div class="get-video-featured-list-text">${obj}</div>
                        </div>`
                    ).join('');
                }
            }
        ...
        // Course Description for Lessons section
        const lessonsDescription = document.querySelector('.course-details-lessons-description-text');
        if (lessonsDescription && course.description) {
            lessonsDescription.textContent = course.description;
        }

        // Lessons/Curriculum
        if (course.lessons || course.syllabus) {
            const lessonsContainer = document.querySelector('.course-details-lessons-video-wrap');
            if (lessonsContainer) {
                const lessonsData = course.lessons || course.syllabus;
                try {
                    const lessons = typeof lessonsData === 'string' ? JSON.parse(lessonsData) : lessonsData;
                    if (Array.isArray(lessons) && lessons.length > 0) {
                        lessonsContainer.innerHTML = lessons.map((lesson, index) =>
                            `<div class="course-details-lessons-card">
                                <div class="course-details-lessons-play-button-details">
                                    <img src="images/Course-Details-Lessons-Play-Button.svg" loading="lazy" alt="" class="course-details-lessons-play-button">
                                    <div class="course-details-lessons-name-details-text">
                                        <div class="course-details-lessons-name">${lesson.title || lesson}</div>
                                        <div class="course-details-lessons-details-text">${lesson.description || 'Course content'}</div>
                                    </div>
                                </div>
                                <div class="course-lessons-time-wrapper">
                                    <div class="course-lessons-time-text">${lesson.duration || '30 Min'}</div>
                                    <img src="images/Course-Details-Lessons-Lock-Icon.svg" loading="lazy" alt="" class="course-lessons-time-icon">
                                </div>
                            </div>`
                        ).join('');
                    }
                    ...
                    if (typeof lessonsData === 'string') {
                        const lessonsList = lessonsData.split(',').map(l => l.trim());
                            ...
                            lessonsContainer.innerHTML = lessonsList.map((lesson, index) =>
                                `<div class="course-details-lessons-card">
                                    <div class="course-details-lessons-play-button-details">
                                        <img src="images/Course-Details-Lessons-Play-Button.svg" loading="lazy" alt="" class="course-details-lessons-play-button">
                                        <div class="course-details-lessons-name-details-text">
                                            <div class="course-details-lessons-name">${lesson}</div>
                                            <div class="course-details-lessons-details-text">Course content</div>
                                        </div>
                                    </div>
                                    <div class="course-lessons-time-wrapper">
                                        <div class="course-lessons-time-text">30 Min</div>
                                        <img src="images/Course-Details-Lessons-Lock-Icon.svg" loading="lazy" alt="" class="course-lessons-time-icon">
                                    </div>
                                </div>`
                            ).join('');
