...
<div role="listitem" class="shared-course-card-item w-dyn-item">
    ...
    /**
     * Static course images mapping (no more dynamic generation!)
     */
    function getStaticCourseImage(course) {
        const staticImages = {
        ...
        return staticImages[course.title] || 'images/placeholder.jpg';
    }
    ...
    async function createCourseCard(course, options = {}) {
        ...
        try {
            ...
            // Filter out bad URLs (about:blank#, #, empty)
            const isValidUrl = course.url &&
            ...
            const courseData = {
                ...
                COURSE_IMAGE: getStaticCourseImage(course),
    ...
    function createFallbackCourseCard(course, options = {}) {
        ...
        courseItem.innerHTML = `
            <div class="featured-courses-single" style="background-color: rgb(4,25,63)">
                <a href="${courseUrl}" class="featured-courses-image-link w-inline-block">
                    <img loading="lazy"
                         src="${getStaticCourseImage(course)}"
                         alt="${course.title}"
                         class="featured-courses-image">
                </a>
                <div class="featured-courses-typography">
                    <div class="featured-courses-name-wrap">
                        <a href="${courseUrl}" class="featured-courses-name course-title-overflow">
                            ${course.title}
                        </a>
