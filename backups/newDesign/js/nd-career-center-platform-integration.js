...
(function() {
    ...
    async function loadCareerCenterPlatformData() {
        try {
            ...
            console.log('API URL:', `${API_BASE_URL}/api/nd/career-center-platform-page`);
    ...
    function populateResourcesSection(resourcesData) {
        ...
        if (resourcesData.title) {
            updateTextContent('[data-section="resources"] [data-field="resources-title"]', resourcesData.title);
        }

        if (resourcesData.subtitle) {
            updateTextContent('[data-section="resources"] [data-field="resources-subtitle"]', resourcesData.subtitle);
        }

        if (resourcesData.description) {
            updateTextContent('[data-section="resources"] [data-field="resources-description"]', resourcesData.description);
        }

        // Populate resource items in existing cards
        if (resourcesData.items && Array.isArray(resourcesData.items)) {
            resourcesData.items.forEach((item, index) => {
                const cardNum = index + 1;

                if (item.title) {
                    updateTextContent(`[data-field="resource-${cardNum}-title"]`, item.title);
                }
                if (item.description) {
                    updateTextContent(`[data-field="resource-${cardNum}-description"]`, item.description);
                }
                if (item.icon) {
                    const iconElement = document.querySelector(`[data-field="resource-${cardNum}-icon"]`);
                    if (iconElement) {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', loadCareerCenterPlatformData);
    } else {
