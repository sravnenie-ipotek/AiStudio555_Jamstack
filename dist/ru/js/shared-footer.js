...
class SharedFooter {
    ...
    getSiteMenuStructure() {
        return [
            ...
            {
                heading: this.getLocalizedText('Company', 'Компания', 'חברה'),
                items: [
                    { content: this.getLocalizedText('About Us', 'О нас', 'אודותינו'), url: 'about-us.html' },
                    { content: this.getLocalizedText('Contact Us', 'Контакты', 'צור קשר'), url: 'contact-us.html' }
                ]
            },
            {
                heading: this.getLocalizedText('Support', 'Поддержка', 'תמיכה'),
                items: [
                    { content: this.getLocalizedText('Help Center', 'Центр помощи', 'מרכז עזרה'), url: 'contact-us.html' },
                    { content: this.getLocalizedText('Terms of Service', 'Условия использования', 'תנאי שימוש'), url: 'template-pages/license.html' },
                    { content: this.getLocalizedText('Privacy Policy', 'Политика конфиденциальности', 'מדיניות פרטיות'), url: 'template-pages/license.html' }
                ]
            }
        ];
    }
