...
(function() {
    ...
    function setupScrollTrigger(awardsSection, innerTicker, cards) {
        ...
        window.addEventListener('scroll', () => {
            if (!scrollTriggerActive || allCardsShown) return;

            clearTimeout(scrollTimeout);
            scrollTimeout = setTimeout(() => {
                console.log('🔄 Updating card overlay due to scroll...');
    ...
    function updateCardOverlay(innerTicker, cards) {
        ...
        // Stop animation when we've shown all cards
        if (cardIndex >= maxCardIndex - 1 && cardProgress >= 0.8) {
